...
import appSettingsFakerFactory from "./appSettingsFakerFactory";
...
const AppSettingsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = appSettingsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("appSettings").create(elem));
...
export default connect(mapState, mapDispatch)(AppSettingsFakerDialogComponent);
